...
```{r, GDP US Graphs, message=FALSE, fig.width=6, fig.hight=6}
...
```{r, Alchol beverage , message=FALSE, fig.width=6, fig.hight=6}  
...
```{r, GDP Graphs, message=FALSE, fig.width=6, fig.hight=6}
...
```{r, GDP Maps, message=FALSE,  fig.width=6, fig.hight=6}
...
We move on to analyse the data of an retailer to gain more information about what customers spend money on wine. 
...
```{r customer food basket, include=FALSE, fig.width=6, fig.height=4}
#ORDER BOXPLOT FROM HIGHEST TO LOWEST
#COLOR WINE BOXPLOT
...
customer_food_basked %>%select(id, wine_prop, fruit_prop, meat_prop, fish_prop, sweet_prop) %>%  
  pivot_longer(cols=2:6, names_to="type", values_to="proportion") %>%  ggplot(aes(y=proportion, x=type, group=type)) + 
  geom_boxplot() + 
  custom_layout + 
...
We know know that the customers of the retailer do appear to purchase a lot ot wine, hence we want to use the data to gain more insights on what characteristics the high wine spenders have, which we could then ultimately use to target our customers. 

```{r, rconsumer personality analysis data 2, fig.width=6, fig.height=6}

#CHANGE ORDER 
#COLOR WIDOW AND DICORED 
#ANNOTATION

#MARITAL STATUS
#We will use median as the data appears to be highly skewed 
profile_clean %>% group_by(marital_status) %>%  summarise(median=median(mnt_wines), count=n()) %>%  filter(marital_status %in% c("Divorced", "Married", "Single","Together", "Widow")) %>% ggplot(aes(x=marital_status, y=median)) + 
  geom_col() + 
  custom_layout +
  labs(title="XXXXXXXX",
...
#EDUCATION
profile_clean %>% group_by(education) %>%  summarise(median=median(mnt_wines), count=n()) %>%  filter(education %in% c("2n Cycle", "Graduation", "Master","PhD")) %>% ggplot(aes(x=education, y=median)) + 
  geom_col() + 
  custom_layout +
  labs(title="XXXXXXXX",
...
#BY CHILDREN AT HOME 
profile_clean %>% group_by(kidhome) %>%  summarise(median=median(mnt_wines), count=n()) %>%   ggplot(aes(x=as.factor(kidhome), y=median)) + 
  geom_col() + 
  custom_layout +
  labs(title="XXXXXXXX",
...
#BY TEEN AT HOME 
profile_clean %>% group_by(teenhome) %>%  summarise(median=median(mnt_wines), count=n()) %>%   ggplot(aes(x=as.factor(teenhome), y=median)) + 
  geom_col() + 
  custom_layout +
  labs(title="XXXXXXXX",
...
profile_clean %>% group_by(Child) %>%  summarise(median=median(mnt_wines), count=n()) %>%   ggplot(aes(x=as.factor(Child), y=median)) + 
  geom_col() + 
  custom_layout +
  labs(title="XXXXXXXX",
       subtitle = "Median wine spending by number of children", 
...
profile_clean %>% group_by(Age) %>%  summarise(median=median(mnt_wines), count=n()) %>%   ggplot(aes(x=as.factor(Age), y=median)) + 
  geom_col() + 
  custom_layout +
  labs(title="XXXXXXXX",
       subtitle = "Median wine spending by age", 
       caption="Source: Customer Personality Analysis Data Set", 
       x= "Age", 
       y= "Median wine spending") 

## ADD MEDIAN WINE SPENDING BY AGE 
## ADD MEDIAN WINE SPENDING BY INCOME 
