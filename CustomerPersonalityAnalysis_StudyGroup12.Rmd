...
```{r create own graphing template, message=FALSE, warning=FALSE, error=FALSE, include=FALSE}
...
```{r, clean pce, message=FALSE, warning=FALSE, error=FALSE, fig.width=8, fig.hight=8}
...
```{r, GDP US Graphs, message=FALSE, fig.width=8, fig.hight=6}
...
```{r, Alchol beverage , message=FALSE, fig.width=8, fig.hight=6}  
...
```{r, wine consumption growth,  message=FALSE, error=FALSE, warning=FALSE, fig.width=6, fig.height=6}
library("readxl")
...
wine_total_us_clean %>%  ggplot(aes(x=year, y=consumption)) + 
 ...
 # add the text label on the graph
  geom_text(
    data = data.frame(x = 6, y = 1100, label = label),
    aes(x = x, y = y, label = label),
    colour="grey15",
    family="PT Sans",
    hjust = 0.5,
...
```{r, wine consumption by state, message=FALSE, warning=FALSE, error=FALSE, fig.width=6, fig.height=6}
library("readxl")
...
```{r customer food basket, warning=FALSE, fig.width=6, fig.height=4}
...
```{r, median age by state, message=FALSE,warning=FALSE, error=FALSE, fig.width=6, fig.height=6}
library("readxl")
...
IS THERE ALSO A WAY TO ONLY INCLUDE SOME OF THE VARIABLES FOR CLUSTREING (AGE; EDUCATION; MARITAL STAUTS; INCOME; CHILDRES) --> only to ones we use in customer analysis  --> happy to talk about this 
